...
class DQNBody(hk.Module):
    ...
    def __call__(self, x):
        ...
        # Apply CNN.
        x = hk.Conv2D(
            32, kernel_shape=8, stride=4, padding="VALID", w_init=w_init
        )(x)
        x = nn.relu(x)
        x = hk.Conv2D(
            64, kernel_shape=4, stride=2, padding="VALID", w_init=w_init
        )(x)
        x = nn.relu(x)
        x = hk.Conv2D(
            64, kernel_shape=3, stride=1, padding="VALID", w_init=w_init
        )(x)
        x = nn.relu(x)
...
class SACDecoder(hk.Module):
    ...
    def __init__(
        self, state_space, num_layers=4, num_filters=32, negative_slope=0.1
    ):
        super().__init__()
