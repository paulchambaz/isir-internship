...
def _from_1d_to_2d(t, v):
    assert len(v) == 1 or len(v) == 2, (
        "projection function outputs must be 1D or 2D"
    )
    if len(v) == 2:
        return 2, v
...
def single_episode_plot(
    ...
    for t, step in enumerate(step_list):
        projection_dimension, x_obs = _from_1d_to_2d(
            t,
            projection_function(step["observation"]),
        )
        projection_dimension, x_obs_next = _from_1d_to_2d(
            t + 1,
            projection_function(step["next_observation"]),
        )
        ...
        rgbs.append(
            (
                1.0 - t / episode_length / 2.0,
                0.2,
                0.2 + t / episode_length / 2.0,
                1,
            )
        )
    ...
    if goalenv:
        if projection_dimension == 2:
            ...
            g_lines = list(zip(g_gather[:-1], g_gather[1:], strict=False))
            ax.add_collection(
