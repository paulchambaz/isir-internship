...
class SAC(OffPolicyActorCritic):
    ...
    def update(self, writer=None):
        ...
        # Update critic.
        self.opt_state_critic, self.params_critic, loss_critic, abs_td = (
            optimize(
                self._loss_critic,
                self.opt_critic,
    ...
    def _calculate_target(
        ...
        next_q = self._calculate_value(
            params_critic_target, next_state, next_action
        )
        next_q -= jnp.exp(log_alpha) * self._calculate_log_pi(
            next_action, next_log_pi
        )
        return jax.lax.stop_gradient(
            reward + (1.0 - done) * self.discount * next_q
        )
    ...
    def _loss_critic(
        ...
        next_state: np.ndarray,
        weight: np.ndarray | list[jnp.ndarray],
        *args,
        **kwargs,
    ) -> tuple[jnp.ndarray, jnp.ndarray]:
    ...
    def _loss_actor(
        ...
        **kwargs,
        ...
        action, log_pi = self._sample_action(
            params_actor, state, *args, **kwargs
        )
    ...
    def load_params(self, save_dir):
        for filename in [
            ...
            self.__dict__[filename] = jax.tree_util.tree_map(
                jnp.array,
                joblib.load(os.path.join(save_dir, filename + ".joblib")),
            )
